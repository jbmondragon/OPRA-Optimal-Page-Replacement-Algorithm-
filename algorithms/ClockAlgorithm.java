...
public class ClockAlgorithm implements Algorithm {
    ...
    public SimulationResult simulate(int[] referenceString, int frameSize) {
        SimulationResult result = new SimulationResult("Clock", referenceString, frameSize);

        ClockFrame[] frames = new ClockFrame[frameSize];
        for (int i = 0; i < frameSize; i++) {
            frames[i] = new ClockFrame(-1);
        }
            ...
            if (!found) {
                ...
                while (true) {
                    if (frames[hand].getPageNumber() == -1) {
                        // Empty frame found
                        frames[hand] = new ClockFrame(page);
    ...
    // Inner class for Clock algorithm frames
    private static class ClockFrame {
        private int pageNumber;
        private boolean referenceBit;

        public ClockFrame(int pageNumber) {
